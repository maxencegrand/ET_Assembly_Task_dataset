...
class TableState:
    ...
    def add_raw(self,timestamp):
        ...
        """
        raw = [timestamp]
        raw.extend(self.initial_state.get_raw(list(range(24))))
    ...
    def extract_initial_state(self):
        ...
        dm = DeviceManager()
        id_0 = df_instruction.loc[df_instruction.index[0], 'block']
        x_0 = df_instruction.loc[df_instruction.index[0], 'x0']
        y_0 = df_instruction.loc[df_instruction.index[0], 'y0']
        x_1 = df_instruction.loc[df_instruction.index[0], 'x1']
        y_1 = df_instruction.loc[df_instruction.index[0], 'y1']
        x_2 = df_instruction.loc[df_instruction.index[0], 'x2']
        y_2 = df_instruction.loc[df_instruction.index[0], 'y2']
        x_3 = df_instruction.loc[df_instruction.index[0], 'x3']
        y_3 = df_instruction.loc[df_instruction.index[0], 'y3']
        for idx in df_blocks.index:
            id = df_blocks.loc[idx, "id"]
            shape = Shape(df_blocks.loc[idx, "shape"])
            color = Color(df_blocks.loc[idx, "color"])
