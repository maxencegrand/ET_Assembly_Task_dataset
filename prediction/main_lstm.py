...
def parsingAllParticipantOneMethode():
    ...
    for seed in liste_seeds:
        # sépare les participants en 80-20
        ...
        # Charge les 10 modeles à notre seed
        ...
        # On utilise les 20% des participants
            ...
            for number in side:
                ...
                for model in os.scandir(participant):
                    ...
                    if os.path.exists(
                        ...
                        # Pour chaque gaze_point
                        for i in range(nb_gaze):
                            temps_debut = time.time()
                            # L'etat du monde actuel
                            ...
                            # Calcul les indicateurs
                            (feature,) = parsingOneSituation(gaze_value)

                            all_feature[:, i, :] = feature

                            input_array = all_feature[
                                :, max(0, i - input_dims + 1) : i + 1, :
                            ]
                            ...
                            if input_array.shape[1] < input_dims:
                                ...
                                input_array = np.pad(
                                    ...
                                    (
                                        *[(0, 0)] * (1),
                                        (input_dims - input_array.shape[1], 0),
                                        *[(0, 0)] * (input_array.ndim - 2),
                                    ),
                                )

                            # Calcul les probabilites des zones de tenon à l'aide de l'approche machine learning
                            for k in range(nb_predi):
                                temps_debut = time.time()
                                new_probability, new_indice = low_level_lstm(
                                    input_array[k],
                                    list_model[method_pos][k],
                                    t,
                                    timestamp_action,
                                    indice,
                                )
                            ...
                            # Calcul la norme entre resultat obtenu et ground truth
                            ...
                            # A partir des probabilites calcul la surface de prediction (l'id de la surface) selon des différentes surfaces
                            (
                                area4max_indices,
                                area8max_indices,
                                area_best_4,
