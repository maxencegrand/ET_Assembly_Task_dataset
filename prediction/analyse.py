...
def analyseSituation(world, history_prediction, timestamp_action):
    ...
    for t in range(1, len(timestamp_action)):
        ...
        for time in range(
            max(0, timestamp_action[t] - 3000),
            min(timestamp_action[-1], timestamp_action[t] + 3001),
        ):
            if (t - 1) % 2 == 0:
...
def analyseRelease(
    quadrillage, liste_good_grasp_zones, liste_good_release_zones, timestamp_action
):
    ...
    for t in range(1, len(timestamp_action)):
        for time in range(
            max(0, timestamp_action[t] - 3000),
            min(quadrillage.shape[1], timestamp_action[t] + 3001),
        ):
            if (t - 1) % 2 == 0:
...
def analyseFixeAreaWeak(
    ...
    for t in range(1, len(timestamp_action)):
        ...
        for time in range(
            max(0, timestamp_action[t] - 3000),
            min(prediction.shape[1], timestamp_action[t] + 3001),
        ):
            if (t - 1) % 2 == 0:
                ...
                x0_block = round(
                    (48 * liste_good_grasp_zones[(t - 1) // 2][0] / largeur)
                )
                y0_block = round(
                    (24 * liste_good_grasp_zones[(t - 1) // 2][1] / hauteur)
                )
                x1_block = round(
                    (48 * liste_good_grasp_zones[(t - 1) // 2][2] / largeur)
                )
                y1_block = round(
                    (24 * liste_good_grasp_zones[(t - 1) // 2][3] / hauteur)
                )
                x2_block = round(
                    (48 * liste_good_grasp_zones[(t - 1) // 2][4] / largeur)
                )
                y2_block = round(
                    (24 * liste_good_grasp_zones[(t - 1) // 2][5] / hauteur)
                )
                x3_block = round(
                    (48 * liste_good_grasp_zones[(t - 1) // 2][6] / largeur)
                )
                y3_block = round(
                    (24 * liste_good_grasp_zones[(t - 1) // 2][7] / hauteur)
                )

                if (
                    (
                        x0_area <= x0_block
                        and x0_block < x2_area
                        and y0_area <= y0_block
                        and y0_block < y2_area
                    )
                    or (
                        x0_area < x1_block
                        and x1_block <= x2_area
                        and y0_area <= y1_block
                        and y1_block < y2_area
                    )
                    or (
                        x0_area < x2_block
                        and x2_block <= x2_area
                        and y0_area < y2_block
                        and y2_block <= y2_area
                    )
                    or (
                        x0_area <= x3_block
                        and x3_block < x2_area
                        and y0_area < y3_block
                        and y3_block <= y2_area
                    )
                ):
                    analyse_grasp[time - (timestamp_action[t] - 3000)] += 1
                nb_analyse_grasp[time - (timestamp_action[t] - 3000)] += 1
                ...
                x0_block = round(
                    (48 * liste_good_release_zones[(t - 1) // 2][0] / largeur)
                )
                y0_block = round(
                    (24 * liste_good_release_zones[(t - 1) // 2][1] / hauteur)
                )
                x1_block = round(
                    (48 * liste_good_release_zones[(t - 1) // 2][2] / largeur)
                )
                y1_block = round(
                    (24 * liste_good_release_zones[(t - 1) // 2][3] / hauteur)
                )
                x2_block = round(
                    (48 * liste_good_release_zones[(t - 1) // 2][4] / largeur)
                )
                y2_block = round(
                    (24 * liste_good_release_zones[(t - 1) // 2][5] / hauteur)
                )
                x3_block = round(
                    (48 * liste_good_release_zones[(t - 1) // 2][6] / largeur)
                )
                y3_block = round(
                    (24 * liste_good_release_zones[(t - 1) // 2][7] / hauteur)
                )
        ...
        for indice in range(24):
            if world[i, 10 * indice + 10] == 1:
                ...
                for zone_id, zone in enumerate(liste_zone):
                    ...
                    dist = math.sqrt(
                        (x_mean - x_centre_zone) ** 2 + (y_mean - y_centre_zone) ** 2
                    )
    ...
    for t in range(1, len(timestamp_action)):
        ...
        for time in range(
            ...
            if (t - 1) % 2 == 0:
                ...
                analyse_grasp[time - (timestamp_action[t] - 3000)] += history_norme[
                    0, time
                ]
                nb_analyse_grasp[time - (timestamp_action[t] - 3000)] += 1
            else:

                analyse_release[time - (timestamp_action[t] - 3000)] += history_norme[
                    1, time
                ]
                nb_analyse_release[time - (timestamp_action[t] - 3000)] += 1
...
def evaluationBestArea(
    prediction, liste_good_grasp_area, liste_good_release_area, timestamp_action
):
    ...
    for t in range(1, len(timestamp_action)):
        for time in range(
            max(0, timestamp_action[t] - 3000),
            min(timestamp_action[-1] - 1, timestamp_action[t] + 3001),
        ):
            ...
            if (t - 1) % 2 == 0:
                ...
                x0 = round(
                    (48 / largeur) * liste_good_release_area[int((t - 1) // 2)][0]
                )
                y0 = round(
                    (24 / hauteur) * liste_good_release_area[int((t - 1) // 2)][1]
                )
                x2 = round(
                    (48 / largeur) * liste_good_release_area[int((t - 1) // 2)][2]
                )
                y2 = round(
                    (24 / hauteur) * liste_good_release_area[int((t - 1) // 2)][3]
                )
