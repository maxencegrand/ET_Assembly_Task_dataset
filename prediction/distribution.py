...
def distri(seed):
    ...
    for method_pos,side in enumerate(left):   
        ...
        sns.histplot(sorted_data, bins=range(int(0), int(max(sorted_data) + binwidth), binwidth), kde=True, color='skyblue')
        ...
        plt.tick_params(axis='x', labelsize=22)  # Taille de la police des ticks sur l'axe des x
