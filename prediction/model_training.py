...
def parsingAllParticipantOneMethode(seed):
    ...
    for method_pos, side in enumerate(left):
        ...
        for k in range(nb_predi):
            ...
            training = np.zeros(
                (nb_action, int(good_size), nb_area_1), dtype=np.float32
            )
                ...
                for model in os.scandir(participant):
                    ...
                    if (
                        ...
                        for t in range(1, gaze_point.shape[0]):
                            ...
                            if (
                                indice < world.shape[0] - 1
                                and t_gaze >= world[indice + 1, 0]
                            ):
                                ...
                                if previous_data.shape[0] < good_size:
                                    previous_data = np.pad(
                                        previous_data,
                                        (
                                            (good_size - previous_data.shape[0], 0),
                                            *[(0, 0)] * (previous_data.ndim - 1),
                                        ),
                                        mode="constant",
                                        constant_values=0,
                                    )
                                ...
                                if world[t, 10 * id_block + 10] == 1:
                                    ...
                                    for x_grasp in range(x0_grasp, x2_grasp):
                                        for y_grasp in range(y0_grasp, y2_grasp):
                                            y_training[compteur_y_event + t - 2][
                                                x_grasp * 24 + y_grasp
                                            ] += (1 / taille_bloc)
                                    ...
                                    for x_release in range(x0_release, x2_release):
                                        for y_release in range(y0_release, y2_release):
                                            y_training[compteur_y_event + t - 1][
            ...
            )  # Choisir l'optimiseur et la fonction de perte appropriés

            # Entraîner le modèle
            history = model.fit(
                training, y_training, epochs=10, batch_size=32, validation_split=0.2
            )
            ...
            with open(
                "history_" + str(seed) + "_" + str(5 * method_pos + k) + ".pkl", "wb"
            ) as f:
